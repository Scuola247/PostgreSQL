...
-- DROP FUNCTION unit_tests.grading_meetings_valutations_iu_trigger(boolean);

CREATE OR REPLACE FUNCTION unit_tests.grading_meetings_valutations_iu_trigger(
...
BEGIN
 ...
 --------------------------------------------------------------
  test_name = 'INSERT grading_meetings_valutations with different classroom';
  --------------------------------------------------------------
  BEGIN
  ...
  BEGIN
    ...
    _results = _results || assert.fail(full_function_name, test_name,'UPDATE was OK but but the student is from a different classroom', NULL::diagnostic.error);
    RETURN;
 ...
 -- 3° TRIGGER 
 -- 1) VOTO IN UN SCRUTINIO GIUSTO , MA DI UNA MATERIA DIVERSA.
 -- 2) VOTO DI UNA MATERIA GIUSTA, MA IN UN SCRUTINIO SBAGLIATO.
 ...
 */
   ...
   BEGIN
    UPDATE public.grading_meetings_valutations SET grading_meeting = '119538000000000' where subject  = '1143000000000';
    _results = _results || assert.fail(full_function_name, test_name,'UPDATE was OK but but the student is from a different classroom', NULL::diagnostic.error);
    RETURN;
    EXCEPTION WHEN SQLSTATE 'U04R5' THEN
      _results = _results || assert.pass(full_function_name, test_name);
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.grading_meetings_valutations_iu_trigger(boolean)
  OWNER TO postgres;
