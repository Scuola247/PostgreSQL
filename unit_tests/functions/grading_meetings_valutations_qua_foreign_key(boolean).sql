-- Function: unit_tests.grading_meetings_valutations_qua_foreign_key(boolean)

-- DROP FUNCTION unit_tests.grading_meetings_valutations_qua_foreign_key(boolean);

CREATE OR REPLACE FUNCTION unit_tests.grading_meetings_valutations_qua_foreign_key(
...
BEGIN
  ...
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23503' THEN
      ...
      IF error.constraint_name = 'grading_meetings_valutations_qua_fk_grade' THEN
  ...
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23503' THEN
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
      IF error.constraint_name = 'grading_meetings_valutations_qua_fk_grading_meeting_valutation' THEN
        _results = _results || assert.pass(full_function_name, test_name);
      ELSE
        _results = _results || assert.fail(full_function_name, test_name, 'Unexpected exception 1', error);   
        RETURN;
  ...
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23503' THEN
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
      IF error.constraint_name = 'grading_meetings_valutations_qua_fk_qualification' THEN
        _results = _results || assert.pass(full_function_name, test_name);
      ELSE
        _results = _results || assert.fail(full_function_name, test_name, 'Unexpected exception 1', error);   
        RETURN;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.grading_meetings_valutations_qua_foreign_key(boolean)
  OWNER TO postgres;
