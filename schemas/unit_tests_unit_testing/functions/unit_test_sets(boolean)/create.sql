-- Function: unit_tests_unit_testing.unit_test_sets(boolean)

-- DROP FUNCTION unit_tests_unit_testing.unit_test_sets(boolean);

CREATE OR REPLACE FUNCTION unit_tests_unit_testing.unit_test_sets(
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        _results = _results || assert.fail(full_function_name, test_name, 'INSERT unit_tests_unit_testing.unit_tests FAILED'::text, error);
        RETURN;
  END;
  RETURN;
END
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests_unit_testing.unit_test_sets(boolean)
  OWNER TO postgres;
