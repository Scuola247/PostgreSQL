...
BEGIN
  ...
  IF _build_dependencies THEN
      PERFORM unit_testing.build_function_dependencies(diagnostic.function_name(context),'unit_tests_security.create_role','unit_tests_public.communication_types');
    RETURN;
  END IF;
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    UPDATE public.communication_types SET description = 'descrizione non esistente' WHERE communication_type = 138012000000000;
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    DELETE FROM public.communication_types WHERE communication_type = 138012000000000;
