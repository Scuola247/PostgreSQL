...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-supervisor@scuola.it';
    UPDATE public.explanations SET description = 'ritardoo' WHERE explanation = '11157313000000000';
    _results = _results || assert.pass(full_function_name, test_name);
 EXCEPTION WHEN SQLSTATE '42501' THEN
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-supervisor@scuola.it';
    DELETE FROM public.explanations WHERE explanation = '11157313000000000';
    _results = _results || assert.pass(full_function_name, test_name);
 EXCEPTION WHEN SQLSTATE '42501' THEN
