...
BEGIN
  ...
  IF _build_dependencies THEN
      PERFORM unit_testing.build_function_dependencies(diagnostic.function_name(context),'unit_tests_security.create_role','unit_tests_public.conversations');
    RETURN;
  END IF;
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    UPDATE public.conversations SET subject = 'subject non esistente' WHERE conversation = '3367000000000';
    _results = _results || assert.pass(full_function_name, test_name);
 EXCEPTION WHEN SQLSTATE '42501' THEN
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    DELETE FROM public.conversations WHERE conversation = '3367000000000';
    _results = _results || assert.pass(full_function_name, test_name);
 EXCEPTION WHEN SQLSTATE '42501' THEN
