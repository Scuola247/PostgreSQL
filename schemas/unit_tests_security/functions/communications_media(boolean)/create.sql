-- Function: unit_tests_security.communications_media(boolean)

-- DROP FUNCTION unit_tests_security.communications_media(boolean);

CREATE OR REPLACE FUNCTION unit_tests_security.communications_media(
...
BEGIN
  ...
  IF _build_dependencies THEN
      PERFORM unit_testing.build_function_dependencies(diagnostic.function_name(context),'unit_tests_security.create_role','unit_tests_public.communications_media');
    RETURN;
  END IF;
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    UPDATE public.communications_media SET description = 'descrizione non esistente' WHERE communication_media = '1120270000000000';
    _results = _results || assert.pass(full_function_name, test_name);
 EXCEPTION WHEN SQLSTATE '42501' THEN
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    DELETE FROM public.communications_media WHERE communication_media = '1120270000000000';
    _results = _results || assert.pass(full_function_name, test_name);
 EXCEPTION WHEN SQLSTATE '42501' THEN
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests_security.communications_media(boolean)
  OWNER TO scuola247_supervisor;
GRANT EXECUTE ON FUNCTION unit_tests_security.communications_media(boolean) TO public;
GRANT EXECUTE ON FUNCTION unit_tests_security.communications_media(boolean) TO scuola247_supervisor WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION unit_tests_security.communications_media(boolean) TO scuola247_user;
GRANT EXECUTE ON FUNCTION unit_tests_security.communications_media(boolean) TO "jiahaodong@gmail.com" WITH GRANT OPTION;
