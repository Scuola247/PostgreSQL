-- Function: unit_tests_security.districts(boolean)

-- DROP FUNCTION unit_tests_security.districts(boolean);

CREATE OR REPLACE FUNCTION unit_tests_security.districts(
...
BEGIN
  ...
  IF _build_dependencies THEN
      PERFORM unit_testing.build_function_dependencies(diagnostic.function_name(context),'unit_tests_security.create_role','unit_tests_public.districts');
    RETURN;
  END IF;
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    UPDATE public.districts SET description = 'descrizione non esistente' WHERE district = '75832100000000';
...
RESET ROLE;
 ...
 BEGIN
    SET ROLE 'test-student-a@scuola-1.it';
    DELETE FROM public.districts WHERE district = '75832100000000';
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests_security.districts(boolean)
