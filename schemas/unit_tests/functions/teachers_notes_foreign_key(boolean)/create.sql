...
BEGIN
  ...
  END IF;  
  ---------------------------------------------------------------------
  test_name = 'UPDATE teachears_notes_fk_classroom set classroom with non existence code';
  ---------------------------------------------------------------------
  BEGIN
    UPDATE teachears_notes SET classroom = '99999' WHERE teacher_notes = '61764000000000'; 
    ...
    EXCEPTION WHEN SQLSTATE '23503' THEN
      ...
      WHEN OTHERS THEN 
        ...
        END;
  ---------------------------------------------------------------------
  test_name = 'UPDATE teachears_notes_fk_student set student with non existence code';
  ---------------------------------------------------------------------
  BEGIN
    UPDATE teachears_notes SET student = '99999' WHERE teacher_notes = '61764000000000'; 
    _results = _results || assert.fail(full_function_name, test_name, 'Update was OK but student set with non existence code', NULL::diagnostic.error);   
    RETURN;
    EXCEPTION WHEN SQLSTATE '23503' THEN
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
  ...
  END;
  ---------------------------------------------------------------------
  test_name = 'UPDATE teachears_notes_fk_teacher set teacher with non existence code';
  ---------------------------------------------------------------------
  BEGIN
    UPDATE teachears_notes SET teacher = '99999' WHERE teacher_notes = '61764000000000'; 
