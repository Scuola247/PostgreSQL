-- Function: unit_tests.grading_meetings_close(boolean)

-- DROP FUNCTION unit_tests.grading_meetings_close(boolean);

CREATE OR REPLACE FUNCTION unit_tests.grading_meetings_close(
...
BEGIN
  ...
  IF _build_dependencies THEN
    PERFORM unit_testing.build_function_dependencies(diagnostic.function_name(context),'grading_meetings_valutations_qua');
    RETURN;
  END IF;  
  ------------------------------------------------
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.grading_meetings_close(boolean)
  OWNER TO postgres;
