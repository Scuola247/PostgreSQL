-- Function: unit_tests.leavings_foreign_key(boolean)

-- DROP FUNCTION unit_tests.leavings_foreign_key(boolean);

CREATE OR REPLACE FUNCTION unit_tests.leavings_foreign_key(
...
BEGIN
 ...
 END IF;  
  --				TROVATI PROBLEMI 
   ---------------------------------------------------------------------
 -- test_name = 'update schools set classroom_student with non existence code';
  ---------------------------------------------------------------------
  -- BEGIN
  --   UPDATE leavings SET classroom_student = '9999999999' WHERE leaving = '58393000000000'; 
 ...
 --   EXCEPTION WHEN SQLSTATE '23503' THEN
  --     GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
   --    IF error.constraint_name = 'leavings_fk_classrooom_student' THEN
  ...
  --       RETURN;
   ...
   --END; 
  ---------------------------------------------------------------------
  test_name = 'update leavings set explanation with non existence code';
  ---------------------------------------------------------------------
  BEGIN
  ...
  END; 
  ---------------------------------------------------------------------
  test_name = 'update leavings set teacher with non existence code';
  ---------------------------------------------------------------------
  BEGIN
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.leavings_foreign_key(boolean)
  OWNER TO postgres;
