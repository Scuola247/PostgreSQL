-- Function: unit_tests.persons_addresses_check(boolean)

-- DROP FUNCTION unit_tests.persons_addresses_check(boolean);

CREATE OR REPLACE FUNCTION unit_tests.persons_addresses_check(
...
BEGIN
  ...
  IF _build_dependencies THEN
    ...
    RETURN;
  END IF;
  --    INSERT INTO public.persons_addresses(person_address,person,address_type,street,zip_code,city) VALUES ('16765000000000','3326000000000','Residence','Via G. Segantini 91','37069','L949');

  ----------------------------------------------------
  test_name = 'duplicate city person street zip_code';
  ----------------------------------------------------
  BEGIN
    ...
    _results = _results || assert.fail(full_function_name, test_name, 'Insert was OK but duplicate city person street zip_code was expected', NULL::diagnostic.error);   
    RETURN;       
  ...
  END; 
  ---------------------------------
  test_name = 'city mandatory';
  ---------------------------------
  BEGIN
    UPDATE persons_addresses SET city = NULL WHERE person_address = '16765000000000';
  ...
  ----------------------------------------
  test_name = 'street''s min lenght';
  ----------------------------------------
  BEGIN
    UPDATE persons_addresses SET street = '  ' WHERE person_address = '16765000000000';
    _results = _results ||  assert.fail(full_function_name, test_name, 'Update was OK but street min lenght was expected', NULL::diagnostic.error);    
    RETURN;   
  ...
  ----------------------------------------
  test_name = 'city''s min lenght';
  ----------------------------------------
  BEGIN
    UPDATE persons_addresses SET city = '  ' WHERE person_address = '16765000000000';
    _results = _results ||  assert.fail(full_function_name, test_name, 'Update was OK but city min lenght was expected', NULL::diagnostic.error);    
    RETURN;   
    EXCEPTION WHEN SQLSTATE '23514' THEN 
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
  ...
  BEGIN
    ...
    _results = _results ||  assert.fail(full_function_name, test_name, 'Update was OK but zip_code min lenght was expected', NULL::diagnostic.error);    
    RETURN;   
    EXCEPTION WHEN SQLSTATE '23514' THEN 
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.persons_addresses_check(boolean)
  OWNER TO postgres;
