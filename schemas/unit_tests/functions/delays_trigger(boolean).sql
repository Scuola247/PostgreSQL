-- Function: unit_tests.delays_trigger(boolean)

-- DROP FUNCTION unit_tests.delays_trigger(boolean);

CREATE OR REPLACE FUNCTION unit_tests.delays_trigger(
...
BEGIN
  ...
  END IF;  
  --------------------------------------------------------------
  test_name = 'UPDATE delays with date on sunday';
  --------------------------------------------------------------
  BEGIN
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.delays_trigger(boolean)
  OWNER TO postgres;
