...
-- DROP FUNCTION unit_tests.out_of_classrooms_trigger(boolean);

CREATE OR REPLACE FUNCTION unit_tests.out_of_classrooms_trigger(
...
*/
  --------------------------------------------------------------
  test_name = 'UPDATE school_operator with a one that has a different school';
  --------------------------------------------------------------
  BEGIN
    UPDATE public.out_of_classrooms set school_operator = '4529000000000' WHERE out_of_classroom = '98577000000000'; 
    _results = _results || assert.fail(full_function_name, test_name,'UPDATE was OK but the school_operator has a different school', NULL::diagnostic.error);
    RETURN;
  ...
  --------------------------------------------------------------
  test_name = 'INSERT school_operator with a one that has a different school';
  --------------------------------------------------------------
  BEGIN
    ...
    _results = _results || assert.fail(full_function_name, test_name,'INSERT was OK but the school_operator has a different school', NULL::diagnostic.error);
    RETURN;
  ...
  --------------------------------------------------------------
  test_name = 'UPDATE school_operator with noone of the roles.';
  --------------------------------------------------------------
  BEGIN
  ...
  --------------------------------------------------------------
  test_name = 'INSERT school_operator with noone of the roles.';
  --------------------------------------------------------------
  BEGIN
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.out_of_classrooms_trigger(boolean)
  OWNER TO postgres;
