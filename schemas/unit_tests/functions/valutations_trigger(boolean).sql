-- Function: unit_tests.grading_meetings_valutations_trigger(boolean)

-- DROP FUNCTION unit_tests.valutations_trigger(boolean);

CREATE OR REPLACE FUNCTION unit_tests.valutations_trigger(
...
--------------------------------------------------------------
  ...
  --------------------------------------------------------------
  test_name = 'UPDATE valutations with teacher that has ''Student'' role';
  --------------------------------------------------------------
  BEGIN
    UPDATE public.valutations SET teacher = '30900000000000' WHERE valutation = '86813000000000';
    _results = _results || assert.fail(full_function_name, test_name,'UPDATE was OK but the teacher is a student', NULL::diagnostic.error);
    RETURN;
  ...
  --------------------------------------------------------------
  test_name = 'INSERT valutations with teacher that has ''Student'' role';
  --------------------------------------------------------------
  BEGIN
    ...
    _results = _results || assert.fail(full_function_name, test_name,'INSERT was OK but the teacher is a student', NULL::diagnostic.error);
    RETURN;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.valutations_trigger(boolean)
  OWNER TO postgres;
