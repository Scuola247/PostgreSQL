-- Function: unit_tests.metrics_foreign_key(boolean)

-- DROP FUNCTION unit_tests.metrics_foreign_key(boolean);

CREATE OR REPLACE FUNCTION unit_tests.metrics_foreign_key(
...
BEGIN
  ...
  ---------------------------------------------------------------------
  test_name = 'update out_of_classrooms set school with a non existing one';
  ---------------------------------------------------------------------
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23503' THEN
      ...
      IF error.constraint_name = 'metrics_fk_school' THEN
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.metrics_foreign_key(boolean)
  OWNER TO postgres;
