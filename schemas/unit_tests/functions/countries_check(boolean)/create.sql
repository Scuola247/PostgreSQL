...
BEGIN
  ...
  -----------------------------------
  BEGIN
    UPDATE countries SET country = NULL WHERE country = '201';
  ...
  -----------------------------------
  BEGIN
    UPDATE countries SET description = NULL WHERE country = '201';
  ...
  END; 
      -----------------------------------
  test_name = 'existing''s mandatory';
  -----------------------------------
  BEGIN
    UPDATE countries SET existing = NULL WHERE country = '201';
    _results = _results || assert.fail(full_function_name, test_name, 'Update was OK but description mandatory was expected', NULL::diagnostic.error);      
    RETURN;
    EXCEPTION WHEN SQLSTATE '23502' THEN 
      _results = _results || assert.pass(full_function_name, test_name);
