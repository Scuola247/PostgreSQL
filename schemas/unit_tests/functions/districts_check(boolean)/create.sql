-- Function: unit_tests.districts_check(boolean)

-- DROP FUNCTION unit_tests.districts_check(boolean);

CREATE OR REPLACE FUNCTION unit_tests.districts_check(
...
BEGIN
  ...
  ----------------------------------------
  BEGIN
    INSERT INTO public.districts(district,description,region) VALUES ('UT','Torino','2');
  ...
  ------------------------------------
  BEGIN
    UPDATE districts SET district = NULL WHERE district = 'TO';
  ...
  ------------------------------------
  BEGIN
    UPDATE districts SET description = NULL WHERE district = 'TO';
  ...
  END; 
  ------------------------------------
  test_name = 'district not empty';
  ------------------------------------
  BEGIN
    UPDATE districts SET district = ' ' WHERE district = 'TO';
    _results = _results || assert.fail(full_function_name, test_name, 'Update was OK but district was empty', NULL::diagnostic.error);     
    RETURN;    
  ...
  ------------------------------------
  BEGIN
    UPDATE districts SET description = ' ' WHERE district = 'TO';
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.districts_check(boolean)
  OWNER TO postgres;
