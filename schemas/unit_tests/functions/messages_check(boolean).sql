-- Function: unit_tests.messages_check(boolean)

-- DROP FUNCTION unit_tests.messages_check(boolean);

CREATE OR REPLACE FUNCTION unit_tests.messages_check(
...
BEGIN
  ...
  END IF;  
  ------------------------------------
  test_name = 'message set NULL';
  ------------------------------------
  BEGIN
    UPDATE messages SET message= NULL WHERE message = '50112000000000';
  ...
  END; 
  ------------------------------------
  test_name = 'written_on set NULL';
  ------------------------------------
  BEGIN
    UPDATE messages SET written_on= NULL WHERE message = '50112000000000';
    _results = _results || assert.fail(full_function_name, test_name, 'Update was OK but mnemonic mandatory was expected', NULL::diagnostic.error);   
    RETURN;
    EXCEPTION WHEN SQLSTATE '23502' THEN 
      _results = _results || assert.pass(full_function_name, test_name);  
  ...
  END; 
  ----------------------------------------
  test_name = 'message_text''s min lenght';
  ----------------------------------------
  BEGIN
    UPDATE messages SET message_text = '' WHERE message = '50112000000000';
    _results = _results ||  assert.fail(full_function_name, test_name, 'Update was OK but message_text min lenght was expected', NULL::diagnostic.error);    
    RETURN;   
    EXCEPTION WHEN SQLSTATE '23514' THEN 
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
      IF error.constraint_name = 'messages_ck_message_text' THEN
        _results = _results || assert.pass(full_function_name, test_name);
     ELSE
        _results = _results || assert.fail(full_function_name, test_name, 'Unexpected exception', error);       
        RETURN;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.messages_check(boolean)
  OWNER TO postgres;
