-- Function: unit_tests.notes_signed_check(boolean)

-- DROP FUNCTION unit_tests.notes_signed_check(boolean);

CREATE OR REPLACE FUNCTION unit_tests.notes_signed_check(
...
BEGIN
  ...
  ---------------------------------
  test_name = 'duplicate notes_signed_uq_note_person (note, person)';
  ---------------------------------
  BEGIN
    ...
    _results = _results || assert.fail(full_function_name, test_name, 'Insert was OK but duplicate notes_signed_uq_note_person was expected', NULL::diagnostic.error);   
    RETURN;        
    EXCEPTION WHEN SQLSTATE '23505' THEN 
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
      IF error.constraint_name = 'notes_signed_uq_note_person' THEN
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.notes_signed_check(boolean)
  OWNER TO postgres;
