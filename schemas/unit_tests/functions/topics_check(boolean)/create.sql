-- Function: unit_tests_public.topics_check(boolean)

-- DROP FUNCTION unit_tests_public.topics_check(boolean);

CREATE OR REPLACE FUNCTION unit_tests_public.topics_check(
...
BEGIN
  ...
  END IF;  
  ------------------------------------
  test_name = 'set topic Null ';
  ------------------------------------
  BEGIN
    UPDATE topics SET topic = NULL WHERE topic ='33242000000000' ;
    _results = _results || assert.fail(full_function_name, test_name, 'Update was OK but topic was expected', NULL::diagnostic.error);   
    RETURN;
  ...
  END; 
  ------------------------------------
  test_name = 'set description Null ';
  ------------------------------------
  BEGIN
  ...
  END; 
  ------------------------------------
  test_name = 'set degree Null ';
  ------------------------------------
  BEGIN
    UPDATE topics SET degree = NULL WHERE topic ='33242000000000' ;
    _results = _results || assert.fail(full_function_name, test_name, 'Update was OK but degree was expected', NULL::diagnostic.error);   
    RETURN;
    EXCEPTION WHEN SQLSTATE '23502' THEN 
      _results = _results || assert.pass(full_function_name, test_name);  
      WHEN OTHERS THEN 
  ...
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23505' THEN 
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
      IF error.constraint_name = 'topics_uq_description' THEN
        _results = _results || assert.pass(full_function_name, test_name); 
      ELSE
        _results = _results || assert.fail(full_function_name, test_name, 'Unexpected exception', error);   
        RETURN;
  ...
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23514' THEN 
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
      IF error.constraint_name = 'topics_ck_description' THEN
        _results = _results || assert.pass(full_function_name, test_name);
     ELSE
        _results = _results || assert.fail(full_function_name, test_name, 'Unexpected exception', error);       
        RETURN;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests_public.topics_check(boolean)
  OWNER TO postgres;
