-- Function: unit_tests.branches_check(boolean)

-- DROP FUNCTION unit_tests.branches_check(boolean);

CREATE OR REPLACE FUNCTION unit_tests.branches_check(
...
BEGIN
  ...
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23505' THEN
      ...
      IF error.constraint_name = 'branches_uq_description' THEN
  ...
  BEGIN
    ...
    EXCEPTION WHEN SQLSTATE '23514' THEN
      GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
      IF error.constraint_name = 'branches_ck_description' THEN
        _results = _results || assert.pass(full_function_name, test_name);
     ELSE
        _results = _results || assert.fail(full_function_name, test_name, 'Unexpected exception', error);
        RETURN;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests.branches_check(boolean)
  OWNER TO postgres;
