...
BEGIN
  ...
  BEGIN 
    PERFORM 1  
	  ...
	  IF FOUND THEN
	    _results = _results ||assert.pass(full_function_name, test_name, format('PASS:  User:%s, School:%s', _user, _school));     
    ELSE 
      _results = _results ||assert.fail(full_function_name, test_name, format('FAIL:  User:%s, School:%s', _user, _school), NULL::diagnostic.error);
      RESET ROLE;
      RETURN;
    END IF;    
 ...
 ----------------------------------------------------
  ...
  BEGIN     
      ...
      JOIN public.classrooms_students cs ON cs.classroom_student = gmv.classroom_student
	    JOIN public.persons p ON p.person = cs.student
	    WHERE p.school = _school;
 ...
 ---------------------------------------------------------------
  ...
  BEGIN 
    PERFORM 1  
      ...
      JOIN public.classrooms_students cs on cs.classroom_student = ooc.classroom_student
      JOIN public.persons p on p.person = cs.student
      WHERE p.school = _school;
  ...
  BEGIN 
    PERFORM 1  
      FROM public.persons_addresses pa
  ...
  BEGIN 
    PERFORM 1  
      FROM public.qualifications_plan qp
