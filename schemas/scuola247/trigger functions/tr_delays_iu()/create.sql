-- Function: scuola247.tr_delays_iu()

-- DROP FUNCTION scuola247.tr_delays_iu();

CREATE OR REPLACE FUNCTION scuola247.tr_delays_iu()
  RETURNS trigger AS
$BODY$
<<me>>
DECLARE
...
--
  SELECT s.school, cs.classroom , cs.student
    ...
    FROM scuola247.delays d
    JOIN scuola247.classrooms_students cs ON cs.classroom_student = d.classroom_student
    JOIN scuola247.classrooms c ON c.classroom = cs.classroom
    JOIN scuola247.school_years sy ON sy.school_year = c.school_year
   WHERE cs.classroom_student = new.classroom_student;
--
-- Check that the on_dates of delay there is at least one lesson
--
  PERFORM 1 
     FROM scuola247.lessons l
  ...
  END IF;
--
-- control that the explanation, if indicated, both related to that student, at that on_dates of delayis within the explanation
--
  IF new.explanation IS NOT NULL THEN

    PERFORM 1 
       FROM scuola247.explanations e
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION scuola247.tr_delays_iu()
  OWNER TO scuola247_supervisor;
GRANT EXECUTE ON FUNCTION scuola247.tr_delays_iu() TO scuola247_supervisor WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION scuola247.tr_delays_iu() TO scuola247_user;
REVOKE ALL ON FUNCTION scuola247.tr_delays_iu() FROM public;
