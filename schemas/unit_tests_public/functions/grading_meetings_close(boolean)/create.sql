...
BEGIN
  ...
  ------------------------------------------------------------------------------
  BEGIN
    _results = _results || assert.check_update_done('UPDATE scuola247.grading_meetings SET closed = ''t'' WHERE grading_meeting = 119533000000000;', 1);
    _results = _results || assert.check_update_done('UPDATE scuola247.grading_meetings SET closed = ''t'' WHERE grading_meeting = 119535000000000;', 1);

    EXCEPTION
       WHEN OTHERS THEN
         GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
         _results = _results || assert.fail(full_function_name, test_name, 'UPDATE scuola247.grading_meetings FAILED'::text, error);
       RETURN;
  END;
  RETURN;
END
