-- Function: unit_tests_public.conversations_invites_check(boolean)

-- DROP FUNCTION unit_tests_public.conversations_invites_check(boolean);

CREATE OR REPLACE FUNCTION unit_tests_public.conversations_invites_check(
...
BEGIN
  ...
  BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
    _results = _results || assert.sqlstate_constraint_equals(me.full_function_name, me.test_name, me.error, '23505','conversations_invites_uq_invited');
	IF unit_testing.last_checkpoint_failed(_results) THEN RETURN; END IF;
  END;
  RETURN;
END
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests_public.conversations_invites_check(boolean)
  OWNER TO postgres;
