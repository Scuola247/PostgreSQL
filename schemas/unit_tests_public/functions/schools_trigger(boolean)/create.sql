-- Function: unit_tests_public.schools_trigger(boolean)

-- DROP FUNCTION unit_tests_public.schools_trigger(boolean);

CREATE OR REPLACE FUNCTION unit_tests_public.schools_trigger(
...
BEGIN
  ...
  BEGIN
    ...
    RETURN;
	/*
    EXCEPTION WHEN SQLSTATE 'U0521' THEN
	...
	*/
	EXCEPTION WHEN OTHERS THEN
		GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
		_results = _results || assert.sqlstate_equals(me.full_function_name, me.test_name, me.error, 'U0521');
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION unit_tests_public.schools_trigger(boolean)
  OWNER TO postgres;
