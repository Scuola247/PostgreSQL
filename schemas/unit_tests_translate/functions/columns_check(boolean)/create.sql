...
BEGIN
  ...
  ----------------------------------------------------------
  test_name = 'DUPLICATE relation translation and language'; 
  ----------------------------------------------------------
  BEGIN
    INSERT INTO translate.columns("column",relation,position,name,language,translation,comment) VALUES ('1299999000000000','299117000000000','2','studen','297479000000000','alunno','Studente con le assenze non giustificate');
    _results = _results || assert.fail(full_function_name, test_name, 'Insert was OK but duplicate relation translation language was expected', NULL::diagnostic.error);
    RETURN;
    EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS error.returned_sqlstate = RETURNED_SQLSTATE, error.message_text = MESSAGE_TEXT, error.schema_name = SCHEMA_NAME, error.table_name = TABLE_NAME, error.column_name = COLUMN_NAME, error.constraint_name = CONSTRAINT_NAME, error.pg_exception_context = PG_EXCEPTION_CONTEXT, error.pg_exception_detail = PG_EXCEPTION_DETAIL, error.pg_exception_hint = PG_EXCEPTION_HINT, error.pg_datatype_name = PG_DATATYPE_NAME;
    _results = _results || assert.sqlstate_constraint_equals(me.full_function_name, me.test_name, me.error, '23505', 'columns_uq_relation_translation_language');
    IF (_results[array_length(_results,1)]).check_point.status = 'Failed' THEN RETURN; END IF; 
  END;
